...
def test_honesty_notes_reflect_populated_posture_and_floor(tmp_path: Path) -> None:
    """Issue #182: on a rerun whose playbook already carries a generated
    Posture and signed Floor invariants, the Honesty section's notes must
    say so — not unconditionally claim "not yet generated by the engine"
    (Posture) / imply zero invariants exist (Floor), which denies the
    existence of content the same playbook contains and, for Floor,
    contradicted the control ladder's "never derived" rule."""
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    _write_trail(out_dir, "deal-alice", ordered_versions=["v1"], provenance="our_paper")
    _write_scope(out_dir, [{"document_id": "deal-alice", "in_scope": True}])
    ...
    playbook["posture"] = {
        "version": 1,
        "system_prompt": "Lean firm on indemnification; trade on payment terms.",
        "generation": {"generated_by": "test", "generated_at": "2026-01-15T09:00:00Z"},
    }
    playbook["floor"] = {
        "invariants": [
            {"id": "floor.cap", "statement": "Liability cap never below 12 months fees."},
            {"id": "floor.law", "statement": "Governing law never outside home jurisdiction."},
        ]
    }
    _write_playbook(out_dir, playbook)

    data = build_after_action_data(out_dir)
    notes = data["honesty"]["honesty_notes"]
    notes_text = " ".join(notes)

    # Must affirmatively state presence, not deny it.
    assert "is present in this playbook" in notes_text
    assert "not yet generated" not in notes_text
    assert "2 human-signed invariant" in notes_text
    assert "none have been signed yet" not in notes_text

    # Reworded per issue #182: Floor is never described as unconditionally
    # "derived" (contradicts SKILL.md's "Posture and Floor are never
    # derived") — only candidates are proposed, humans sign.
    assert "Floor clauses are derived from classified reversals" not in notes_text

    report = build_after_action_report(out_dir)
    assert "is present in this playbook" in report
    assert "2 human-signed invariant" in report


def test_honesty_reversal_count_line_does_not_assert_derivation(tmp_path: Path) -> None:
    """Issue #182 fix round 1: the reversal-count sentence in the rendered
    Honesty section (_render_honesty) must not repeat the "Floor derivation
    requires attorney classification" framing rejected elsewhere in this
    section (SKILL.md: "Posture and Floor are never derived"). On a
    playbook whose Floor already carries signed invariants, this line must
    not contradict the honesty_notes floor_note immediately above it by
    implying Floor is still unpopulated."""
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    _write_trail(out_dir, "deal-alice", ordered_versions=["v1"], provenance="our_paper")
    _write_scope(out_dir, [{"document_id": "deal-alice", "in_scope": True}])
