...
# ---------------------------------------------------------------------------
# view document — DEPRECATED alias of view bundle
# ---------------------------------------------------------------------------


def test_view_document_is_alias_of_bundle(tmp_path: Path) -> None:
    """The alias writes the bundle and never the retired document artifact."""
    _make_opf(tmp_path)
    runner = CliRunner()
    result = runner.invoke(cli, ["view", "document", str(tmp_path / "out")])
    assert result.exit_code == 0, result.output

    assert (tmp_path / "out" / "playbook.opf.html").exists()
    assert not (tmp_path / "out" / "playbook.document.html").exists()

    # byte-identical to `view bundle` — it is the same artifact, not a variant
    alias_html = (tmp_path / "out" / "playbook.opf.html").read_text(encoding="utf-8")
    (tmp_path / "out" / "playbook.opf.html").unlink()
    result = runner.invoke(cli, ["view", "bundle", str(tmp_path / "out")])
    assert result.exit_code == 0, result.output
    assert alias_html == (tmp_path / "out" / "playbook.opf.html").read_text(encoding="utf-8")


def test_view_document_prints_deprecation_note(tmp_path: Path) -> None:
    _make_opf(tmp_path)
    runner = CliRunner()
    result = runner.invoke(cli, ["view", "document", str(tmp_path / "out")])
    assert result.exit_code == 0, result.output
    assert "deprecated" in result.output
    assert "view bundle" in result.output


def test_view_document_rejects_alias_map(tmp_path: Path) -> None:
    """The bundle embeds canonical pseudonymized JSON — an alias map would
    leak real names into the shareable artifact and break hash verification.
    The alias must refuse and point at the internal annotation surface."""
    _make_opf(tmp_path)
    alias_path = tmp_path / "alias_map.json"
    alias_path.write_text(json.dumps({"Counterparty-1": "Real Uni"}), encoding="utf-8")
    runner = CliRunner()
    result = runner.invoke(
        cli,
        ["view", "document", str(tmp_path / "out"), "--alias-map", str(alias_path)],
    )
    assert result.exit_code != 0
    assert "does not accept --alias-map" in result.output
    assert "view render" in result.output
    # refused before writing anything, and no real name reached disk
    assert not (tmp_path / "out" / "playbook.opf.html").exists()
    assert not (tmp_path / "out" / "playbook.document.html").exists()
    stored = (tmp_path / "out" / "playbook.opf.json").read_text(encoding="utf-8")
    assert "Real Uni" not in stored


def test_view_document_custom_out_and_missing_opf(tmp_path: Path) -> None:
    _make_opf(tmp_path)
    custom = tmp_path / "pretty.html"
    runner = CliRunner()
    result = runner.invoke(cli, ["view", "document", str(tmp_path / "out"), "--out", str(custom)])
    assert result.exit_code == 0, result.output
    assert custom.exists()
    # still the bundle, just at a caller-chosen path
    assert '<script id="opf-canonical" type="application/json">' in custom.read_text(
        encoding="utf-8"
    )

    empty = tmp_path / "empty"
    empty.mkdir()
    result = runner.invoke(cli, ["view", "document", str(empty)])
    assert result.exit_code != 0


# view render keeps its --alias-map support (the internal annotation surface
# the rejection message above redirects to); covered by
# test_view_render_cmd_with_alias_map_resolves_names.
