...
def _build_honesty(
                ...
                blank_fields.append(
                    {
                        ...
                        "field": "floor",
                        "reason": (
                            "Floor invariants are proposed from classified reversals; "
                            "only human-signed invariants enter the playbook — none "
    ...
    # Issue #182: these notes must reflect THIS run's playbook state, not
    # assert a fixed "nothing here yet" story on every report regardless of
    # what was actually compiled — a rerun with a signed Posture/Floor was
    # being told by its own Honesty section that neither existed.
    posture_present = bool((playbook.get("posture") or {}).get("system_prompt"))
    floor_invariants = (playbook.get("floor") or {}).get("invariants") or []
    floor_present = bool(floor_invariants)
    ...
    # Reworded per issue #182: the control ladder never derives Floor
    # invariants automatically (SKILL.md: "Posture and Floor are never
    # derived") — only proposes candidates from classified reversals for a
    # human to accept. State the actual invariant count when present instead
    # of unconditionally claiming none exist.
    if floor_present:
        floor_note = (
            f"Floor carries {len(floor_invariants)} human-signed invariant(s). "
            "Floor candidates are proposed from classified reversals; only "
            "human-signed invariants ever enter the playbook."
        )
    else:
        floor_note = (
            "Floor candidates are proposed from classified reversals; only "
            "human-signed invariants enter the playbook — none have been "
            "signed yet."
        )
    ...
    return {
        ...
        "honesty_notes": [posture_note, floor_note],
    }
...
def _render_honesty(data: dict[str, Any]) -> list[str]:
    ...
    if rev > 0:
        # Issue #182: this line reproduced the same "Floor derivation"
        # framing rejected elsewhere in this section (SKILL.md: "Posture
        # and Floor are never derived") and, on a playbook whose Floor
        # already carries signed invariants, directly contradicted the
        # honesty_notes floor_note above it. Reworded to "proposed... for
        # human review" and to reflect signed invariants when present.
        floor_signed = data.get("floor_invariant_count", 0)
        if floor_signed:
            lines.append(
                f"**{rev} reversal(s)** detected — Floor candidates are proposed "
                f"from classified reversals for human review "
                f"({floor_signed} invariant(s) already signed)."
            )
        else:
            lines.append(
                f"**{rev} reversal(s)** detected — Floor candidates are proposed "
                "from classified reversals for human review; none have been "
                "signed yet."
            )
        lines.append("")
